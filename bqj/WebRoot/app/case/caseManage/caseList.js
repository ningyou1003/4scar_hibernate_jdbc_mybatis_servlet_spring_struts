...
THISPAGE = {
	...
	loadGrid: function (){
		...
		//从cookie获取页码
		if (vm.type=='big') {
			var casePage = $.cookie('casePageNumberBig');
			//console.log("---"+casePage);
			casePage==null?1:casePage;
		}else if(vm.type=='other'){
			var casePage = $.cookie('casePageNumberOther');
			//console.log("---"+casePage);
			casePage==null?1:casePage;
		}else if(vm.type=='book'){
			var casePage = $.cookie('casePageNumberBook');
			//console.log("---"+casePage);
			casePage==null?1:casePage;
		}
	...
	initPage: function (pageData) {
        ...
        $("#PageInfo").pagePlugin({
            ...
            requst: function (index) {
            	...
            	//从cookie获取页码
        		if (vm.type=='big') {
        			$.cookie('casePageNumberBig', index, {path: '/'});
        		}else if(vm.type=='other'){
        			$.cookie('casePageNumberOther', index, {path: '/'});
        		}else if(vm.type=='book'){
        			$.cookie('casePageNumberBook', index, {path: '/'});
        		}
	...
	addEvent : function() {
		$("#add").click(function() {
			window.location = "caseDetail.html?type="+vm.type;
...
function edit(keyid){
	window.location = "caseDetail.html?keyid=" + keyid+"&type="+vm.type;
}
