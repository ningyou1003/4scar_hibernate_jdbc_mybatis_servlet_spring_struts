...
/**
 * A data access object (DAO) providing persistence and search support for Dict
...
public class DictDAO extends BaseDAO {
	...
	public Dict findById(java.lang.Long id) {
		log.debug("getting Dict instance with id: " + id);
		try {
			Dict instance = (Dict) getSession().get("com.zrcx.entity.Dict", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Dict instance) {
		...
		try {
			List results = getSession().createCriteria("com.zrcx.entity.Dict")
	...
	public Dict merge(Dict detachedInstance) {
		...
		try {
			Dict result = (Dict) getSession().merge(detachedInstance);
