...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var noticePage = $.cookie('noticePageNumber');
		noticePage==null?1:noticePage;
		
		Public.ajaxPost(Public.rootPath() + '/notice/noticeList', {
			'page' : noticePage,
			'NoticeSelectVO' : vm.select.$model
			}, function(json) {
			if (json.status == 001) {
	...
	initPage : function(pageData) {
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				// 存储页码到cookie 
            	$.cookie('noticePageNumber', index, {path: '/'}); 
