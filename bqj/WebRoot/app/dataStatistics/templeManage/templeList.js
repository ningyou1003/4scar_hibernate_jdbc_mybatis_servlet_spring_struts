...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var templePage = $.cookie('templePageNumber');
		templePage==null?1:templePage;
		
		Public.ajaxPost(Public.rootPath() + '/temple/List', {
			'page' : templePage,
	...
	initPage : function(pageData) {
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				// 存储页码到cookie 
            	$.cookie('templePageNumber', index, {path: '/'});
...
/*		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "templeAdd.html?keyid=" + vm.selectId[0];
				} else {
		...
		$("#add").click(function(t) {// 添加
			window.location = "templeAdd.html" ;
		});
