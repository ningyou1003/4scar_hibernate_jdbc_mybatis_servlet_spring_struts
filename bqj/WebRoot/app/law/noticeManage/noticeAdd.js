...
THISPAGE = {
	...
	initData : function() {
		...
		if (request.queryString["keyid"] != null
			...
			Public.ajaxPost(Public.rootPath() + "/notice/editData", "keyId="
...
function postData() {
	...
	if(vm.data.keyid != "" && vm.data.keyid != null&& vm.data.keyid !=undefined){
		url= Public.rootPath() + "/notice/edit";
	}else{
		url= Public.rootPath() + "/notice/add";
	}
	Public.ajaxPost(url, vm.data.$model, function(
		...
		if (json.status == 001) {
			top.openeasydialog("success", json.msg, function(item, dialog){
				dialog.close();
				location.href=htmlRootPath+"/app/law/noticeManage/noticeList.html";
			});
		} else {
			top.openeasydialog("error", json.msg);
		}
...
$(document).ready(function(e) {
	...
	KE();
	THISPAGE.init();
	initUpload("notice", vm.data.keyid);
});
