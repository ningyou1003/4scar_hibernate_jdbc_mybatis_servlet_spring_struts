    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        getCols: function () {
            ...
            for (var id in g._columns) {
                var col = g._columns[id];
                if (col.name != undefined) cols.push({ name: col.name, type: col.type });
            }
            return cols;
        },
        //获取列集合 
        getCanEidtCols: function () {
            var g = this, p = this.options;
            var cols = [];
            for (var id in g._columns) {
                var col = g._columns[id];
                if (col.name != undefined && col.editor != undefined) cols.push({ name: col.name, type: col.type });
            }
            return cols;
        },
        //改变排序
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            var g = this, p = this.options;
            var gridhtmlarr = [];
            for (var rowparm = 0;rowparm < data.length; rowparm++) {
                var item = data[rowparm];
                var rowid = item['__id'];
                if (!item) continue;
                gridhtmlarr.push('<tr');
                ...
                $(g.columns).each(function (columnindex, column) {
                    ...
                    if (this.isrownumber) {
                        ...
                        if (p.lxrownumbers) gridhtmlarr.push('>' + (parseInt(item['__index']) + 1 + ((p.page - 1) * (parseInt(p.pageSize)))) + '</div></td>');
                        else gridhtmlarr.push('>' + (parseInt(item['__index']) + 1) + '</div></td>');
                        return;
                    }
                    //如果是复选框(系统列)
                    if (this.ischeckbox) {
        ...
        _getTotalCellContent: function (column, data) {
            ...
            if (column.totalSummary) {
                ...
                if (column.totalSummary.render) {
                    ...
                    if (isExist('sum'))
                        if (column.type == "int") totalsummaryArr.push("<div>" + sum + "</div>");
                        else totalsummaryArr.push("<div>" + sum.toFixed(2) + "</div>");
                    if (isExist('count'))
        ...
        _setEvent: function () {
			...
			//为了适应ＩＥ８下的LigerUI控件和avalon的兼容,不知道为什么在这里注册的点击事件无法触发
