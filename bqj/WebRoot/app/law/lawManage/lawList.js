...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var lawPage = $.cookie('lawPageNumber');
		lawPage==null?1:lawPage;
		
		Public.ajaxPost(Public.rootPath() + '/law/List', {
			'page' : lawPage,
	...
	initPage : function(pageData) {
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				// 存储页码到cookie 
            	$.cookie('lawPageNumber', index, {path: '/'});
...
/*		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "lawAdd.html?keyid=" + vm.selectId[0];
				} else {
