...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var showPage = $.cookie('showPageNumber');
		showPage==null?1:showPage;
		
		Public.ajaxPost(Public.rootPath() + '/show/List', {
			'page' : showPage,
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				Public.ajaxPost(Public.rootPath() + '/show/List', {
					...
					// 存储页码到cookie 
	            	$.cookie('showPageNumber', index, {path: '/'}); 
...
/*		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "showAdd.html?keyid=" + vm.selectId[0];
				} else {
