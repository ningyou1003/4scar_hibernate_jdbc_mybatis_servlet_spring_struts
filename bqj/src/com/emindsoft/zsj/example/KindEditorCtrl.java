...
@ControllerKey("ke")
public class KindEditorCtrl extends AdminBaseController {
	...
	/*public void fileUpload() throws ServletException, IOException,
		...
		if (!ServletFileUpload.isMultipartContent(request)) {
			success1(getError("请选择文件。"));
		}
		// 检查目录
		File uploadDir = new File(savePath);
		if (!uploadDir.isDirectory()) {
			success1(getError("上传目录不存在。"));
		}
		// 检查目录写权限
		if (!uploadDir.canWrite()) {
			success1(getError("上传目录没有写权限。"));
		}
		...
		while (itr.hasNext()) {
			...
			if (!item.isFormField()) {
				...
				if (!Arrays.<String> asList(extMap.get(dirName).split(","))
						.contains(fileExt)) {
					success1(getError("上传文件扩展名是不允许的扩展名。\n只允许"
				...
				try {
					...
					success1(getError("上传文件失败。"));
				}

				Map<String, Object> msg = new HashMap<String, Object>();
				msg.put("error", 0);
				msg.put("url", saveUrl + newFileName);
				this.render(new JsonRender(msg).forIE());
			}
		}
	}*/
	...
	public void fileUpload() throws ServletException, IOException,
		...
		HttpServletRequest request = getRequest();
		HttpServletResponse response = getResponse();
		String savePath = PropertiesContent.get("upload_dir");
//		String rootDir = savePath;

		// 文件保存目录URL
//		 String saveUrl ="http://"+ request.getLocalAddr()+":"+request.getLocalPort() + "/attachment/ke_upload/";
		String outer_net = PropertiesContent.get("outer_net");
		outer_net = outer_net.substring(0, outer_net.lastIndexOf("/"));
		String saveUrl = outer_net + "/attachment/ke_upload/";
...
//				String path = savePath.substring(rootDir.length(),
//						savePath.length());
				try {
					File uploadedFile = new File(savePath, newFileName);
					item.write(uploadedFile);
...
//					a.set("relateType", "kindEditorAttach")
//							.set("path", path + newFileName)
//							.set("name", fileName)
...
//						+ "/ke/download?attachmentId=";
//				msg.put("url", saveUrl + a.getStr("keyid"));
				msg.put("url", saveUrl + newFileName);
				this.render(new JsonRender(msg).forIE());
			}
		}
