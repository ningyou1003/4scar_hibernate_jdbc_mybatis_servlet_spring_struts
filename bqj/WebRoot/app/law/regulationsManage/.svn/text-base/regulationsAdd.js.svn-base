...
THISPAGE = {
	...
	initEvent : function() {
		$("#btback").click(function() {
			window.location = "regulationsList.html";
		});
	},
	initData : function() {
		...
		if (request.queryString["keyid"] != null
			...
			Public.ajaxPost(Public.rootPath() + "/regulations/editData", "keyId="
...
function postData() {
	...
	if(vm.data.keyid != "" && vm.data.keyid != null&& vm.data.keyid !=undefined){
		url= Public.rootPath() + "/regulations/edit";
	}else{
		url= Public.rootPath() + "/regulations/add";
	}
	Public.ajaxPost(url, vm.data.$model, function(
		...
		if (json.status == 001) {
			top.openeasydialog("success", json.msg, function(item, dialog){
				dialog.close();
				location.href=htmlRootPath+"/app/law/regulationsManage/regulationsList.html";
			});
		} else {
			top.openeasydialog("error", json.msg);
		}
...
$(document).ready(function(e) {
	...
	KE();
	THISPAGE.init();
	initUpload("regulations", vm.data.keyid);
});
