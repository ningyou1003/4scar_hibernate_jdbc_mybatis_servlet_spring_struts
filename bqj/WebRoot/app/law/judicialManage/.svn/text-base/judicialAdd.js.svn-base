...
function postData() {
	...
	if(vm.data.keyid != "" && vm.data.keyid != null&& vm.data.keyid !=undefined){
		url= Public.rootPath() + "/judicial/edit";
	}else{
		url= Public.rootPath() + "/judicial/add";
	}
	Public.ajaxPost(url, vm.data.$model, function(
		...
		if (json.status == 001) {
			top.openeasydialog("success", json.msg, function(item, dialog){
				dialog.close();
				location.href=htmlRootPath+"/app/law/judicialManage/judicialList.html";
			});
		} else {
			top.openeasydialog("error", json.msg);
		}
...
$(document).ready(function(e) {
	...
	KE();
	THISPAGE.init();
	initUpload("judicial", vm.data.keyid);
});
