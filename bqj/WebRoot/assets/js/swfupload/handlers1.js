...
function imgQueued(file){
	addReadyImgInfo(file.id,file.name,"成功加载到上传队列");
}
...
function addReadyFileInfo(fileid,fileName,message,status){
	//用表格显示
	/*获取应用域相应的name，根据name决定往那块域插入表格*/
	var name  = $(".sel_div table").attr("name");
	if(name!=null&&"table0"==name){
		var infoTable0 = document.getElementById("infoTable0");
		//插入行
		var row = infoTable0.insertRow();
		row.id = fileid;
		//插入列
		...
		col3.style.width="80";
		col4.style.width="50";
	}else if(name!=null&&"table1"==name){
		var infoTable1 = document.getElementById("infoTable1");
		var row1 = infoTable1.insertRow();
		row1.id = fileid;
		
		var col11 = row1.insertCell();
		var col12 = row1.insertCell();
		var col13 = row1.insertCell();
		var col14 = row1.insertCell();
		col14.align = "right";
		col11.innerHTML = message+" : ";
		col12.innerHTML = fileName;
		if(status!=null&&status!=""){
			col13.innerHTML="<font color='red'>"+status+"</font>";
		}else{
			col13.innerHTML="";
		}
		col14.innerHTML = "<a href='javascript:deleteFile1(\""+fileid+"\")'>删除</a>";
		col11.style.width="150";
		col12.style.width="250";
		col13.style.width="80";
		col14.style.width="50";
	}else if(name!=null&&"table2"==name){
		var infoTable2 = document.getElementById("infoTable2");
		var row = infoTable2.insertRow();
		row.id = fileid;
		var col21 = row.insertCell();
		var col22 = row.insertCell();
		var col23 = row.insertCell();
		var col24 = row.insertCell();
		col24.align = "right";
		col21.innerHTML = message+" : ";
		col22.innerHTML = fileName;
		if(status!=null&&status!=""){
			col23.innerHTML="<font color='red'>"+status+"</font>";
		}else{
			col23.innerHTML="";
		}
		col24.innerHTML = "<a href='javascript:deleteFile2(\""+fileid+"\")'>删除</a>";
		col21.style.width="150";
		col22.style.width="250";
		col23.style.width="80";
		col24.style.width="50";
	}else{
		var infoTable3 = document.getElementById("infoTable3");
		var row = infoTable3.insertRow();
		row.id = fileid;
		var col31 = row.insertCell();
		var col32 = row.insertCell();
		var col33 = row.insertCell();
		var col34 = row.insertCell();
		col34.align = "right";
		col31.innerHTML = message+" : ";
		col32.innerHTML = fileName;
		if(status!=null&&status!=""){
			col33.innerHTML="<font color='red'>"+status+"</font>";
		}else{
			col33.innerHTML="";
		}
		col34.innerHTML = "<a href='javascript:deleteFile3(\""+fileid+"\")'>删除</a>";
		col31.style.width="150";
		col32.style.width="250";
		col33.style.width="80";
		col34.style.width="50";
	}
	
}
function addReadyImgInfo(fileid,fileName,message,status){
	//用表格显示
	...
	col4.innerHTML = "<a href='javascript:deleteVideoImg(\""+fileid+"\")'>删除</a>";
	col1.style.width="150";
	col2.style.width="250";
	col3.style.width="80";
	col4.style.width="50";
...
function cancelUpload(){
	var infoTable = document.getElementById("infoTable0");
...
function deleteVideoImg(fileId){
	//用表格显示
	var infoTable = document.getElementById("infoTables");
	var row = document.getElementById(fileId);
	infoTable.deleteRow(row.rowIndex);
