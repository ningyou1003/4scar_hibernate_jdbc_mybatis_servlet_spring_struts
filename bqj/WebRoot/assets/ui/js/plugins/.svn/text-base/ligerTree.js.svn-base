...
(function ($) {
    ...
    $.ligerDefaults.Tree = {
        ...
        onClick: function () { },
        idFieldName: 'id',
        seleItem: '0',           //判断是否需要默认选中某一项让其变颜色null为不需要,自加
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        //选择节点(参数：条件函数、Dom节点或ID值,第二个为是否需要点击响应)
        selectItem: function (selectNodeParm) {
            var g = this, p = this.options;
            var clause = null;
...
//                g.trigger('select', [{ data: treenodedata, target: treeitembody.parent().get(0)}]);//无需触发事件，仅仅改变颜色就好
                return;
            }
            else {
        ...
        _upadteTreeWidth: function () {
            var g = this, p = this.options;
            var treeWidth = g.maxOutlineLevel * 50;
            if (p.checkbox) treeWidth += 50;
            if (p.parentIcon || p.childIcon) treeWidth += 50;
