...
public class Big extends BaseModel<Big>{
	...
	public Page<Big> page(int pageNo, int pageSize,String type){
		return this.paginate(pageNo, pageSize, "SELECT *"," FROM c_case WHERE type='" + type + "' and status<>0 AND IsPublic = 1 ORDER BY Time DESC");
	}
	...
	public Page<Big> pageAndroid(int pageNo, int pageSize,String type,String regionId){
		return this.paginate(pageNo, pageSize, "SELECT *"," FROM c_case WHERE type='" + type + "' AND IsPublic = 1 and status<>0 AND regionId='" + regionId + "' ORDER BY Time DESC");
	}
}
