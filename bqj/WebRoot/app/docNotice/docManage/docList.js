...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var docPage = $.cookie('docPageNumber');
		docPage==null?1:docPage;
		
		Public.ajaxPost(Public.rootPath() + '/doc/docList', {
			'page' : docPage
			}, function(json) {
			if (json.status == 001) {
	...
	initPage : function(pageData) {
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				// 存储页码到cookie 
            	$.cookie('docPageNumber', index, {path: '/'}); 
		...
		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "docAdd.html?keyid=" + vm.selectId[0];
				} else {
...
function approve(keyid){
	flowSend("document", keyid);
}
function issued(keyid){
	$.cookie("issued",true,{ path: '/' });
	flowissued("document", keyid);
}

function summation(){
	
	Public.ajaxPost(Public.rootPath()
			+ "/doc/summation", [], function(json) {
		if (json.status == 001) {
			var h="";
