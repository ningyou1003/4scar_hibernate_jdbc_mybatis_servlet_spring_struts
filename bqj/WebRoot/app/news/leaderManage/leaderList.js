...
THISPAGE = {
	...
	loadGrid: function (){
		...
		//从cookie获取页码
		var leaderPage = $.cookie('leaderPageNumber');
		leaderPage==null?1:leaderPage;
		
		Public.ajaxPost(Public.rootPath() + '/leader/listPage', {
			'page': leaderPage,
            'LeaderSelectVO': vm.select.$model,
            'month':month,
	...
	initPage: function (pageData) {
        ...
        $("#PageInfo").pagePlugin({
            ...
            requst: function (index) {
            	// 存储页码到cookie 
            	$.cookie('leaderPageNumber', index, {path: '/'}); 
	...
	addEvent : function() {
		$("#add").click(function() {
			window.location = "leaderDetail.html";
		...
		$("#edit").click(function(t) {
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "leaderDetail.html?keyid=" + vm.selectId[0];
				} else {
			...
			if (vm.selectId.length == 0) {
				top.openeasydialog("warn","请选择要删除的内容");
					...
					top.openeasydialog("warning", "确认要删除吗？",function(yes){
	            		if(yes){
	            			var url = "";
	            			url=Public.rootPath() + "/leader/delete";	            			
