...
THISPAGE = {
	...
	initEvent : function() {
		$("#btback").click(function() {
			window.location = "buildList.html";
		});
	},
	initData : function() {
		...
		if (request.queryString["keyid"] != null
			...
			Public.ajaxPost(Public.rootPath() + "/build/editData", "keyId="
...
function postData() {
	...
	if(vm.data.keyid != "" && vm.data.keyid != null&& vm.data.keyid !=undefined){
		url= Public.rootPath() + "/build/edit";
	}else{
		url= Public.rootPath() + "/build/add";
	}
	Public.ajaxPost(url, vm.data.$model, function(
		...
		if (json.status == 001) {
			top.openeasydialog("success", json.msg, function(item, dialog){
				dialog.close();
				location.href=htmlRootPath+"/app/build/buildManage/buildList.html";
			});
		} else {
			top.openeasydialog("error", json.msg);
		}
...
$(document).ready(function(e) {
	...
	KE();
	THISPAGE.init();
	initUpload("build", vm.data.keyid);
});
