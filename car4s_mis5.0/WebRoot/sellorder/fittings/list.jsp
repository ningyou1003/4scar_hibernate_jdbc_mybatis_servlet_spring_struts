...
<body>
    <div class="alert alert-info">
                   当前位置<b class="tip"></b>销售管理<b class="tip"></b>配件销售单列表
    </div>
    <form id="form01" action="${_cxt}/fitSellorder_list.do"  method="post">
    <!-- 隐藏域 用于判断是不是提交表单-->
    ...
    <table class="tbform list">
          ...
          <tbody>
           <tr>
	           ...
	           <td align="left">
	             <s:textfield name="fitSellorder.brand" cssClass="ipt"/>
	           </td>
	           <td align="right">配件名称:</td>
	           <td align="left">
	             <s:textfield name="fitSellorder.fitName" cssClass="ipt"/>
	           </td>
         </tr>
         <tr>
           <td align="right">提货状态:</td>
           <td align="left">
             <s:select name="fitSellorder.outState" 
                       list="#application.FITTINGS_OUT_STATE" cssClass="ipt"
         ...
         <tr>
           ...
           <td align="left">
                   ...
                   <input class="btn" type="button" value="新增" 
                          onclick="location.href='${_cxt}/sellorder/fittings/add.jsp'"/>
                </td>
            </tr>
          </tbody>
      </table>
    <table class="tb">
        ...
        <tbody>
		    ...
		    <s:iterator  value="#request.list" var="v" status="status">
		     <!-- 删除链接变量 -->
		     <s:url id ="delete" action="fitSellorder_delete">
		        <s:param name="fitSellorder.id">${v.id}</s:param>
		     </s:url>
		     <!-- 更新链接变量 -->
		     <s:url id ="update" action="fitSellorder_update">
		        <s:param name="fitSellorder.id">${v.id}</s:param>
		     </s:url>	     
		     <tr <s:if test="#status.even">class='even'</s:if>>
		        ...
		        <td>
		           <mt:tran data="${applicationScope.FITTINGS_OUT_STATE}" value="${v.outState}"/>
		        </td>		        
		        <td>
		        <s:if test="#v.outState == 1">
	              ...
	              <s:a href="%{update}" cssClass="btn btn-mini btn-primary">提货</s:a> 
		        </s:if>
	            <s:else>无操作</s:else>
		        </td>
	     </tr>
