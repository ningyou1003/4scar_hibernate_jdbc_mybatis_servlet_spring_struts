...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var mediaPage = $.cookie('mediaPageNumber');
		mediaPage==null?1:mediaPage;
		
		Public.ajaxPost(Public.rootPath() + '/media/List', {
			'page' : mediaPage,
	...
	initPage : function(pageData) {
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				// 存储页码到cookie 
            	$.cookie('mediaPageNumber', index, {path: '/'});
...
/*		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "mediaAdd.html?keyid=" + vm.selectId[0];
				} else {
