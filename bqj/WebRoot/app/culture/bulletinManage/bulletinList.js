...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var bulletinPage = $.cookie('bulletinPageNumber');
		bulletinPage==null?1:bulletinPage;
		
		Public.ajaxPost(Public.rootPath() + '/bulletin/List', {
			'page' : bulletinPage,
	...
	initPage : function(pageData) {
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				// 存储页码到cookie 
            	$.cookie('bulletinPageNumber', index, {path: '/'}); 
            	
				Public.ajaxPost(Public.rootPath() + '/bulletin/List', {
					'page' : index,
					'BulletinSelectVO' : vm.select.$model,
				}, function(json) {
					if (json.status == 001) {
			        	$("#checkalls").removeAttr("checked");
						vm.data = json.data.page.list;
...
/*		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "bulletinAdd.html?keyid=" + vm.selectId[0];
				} else {
		...
		$("#add").click(function(t) {// 添加
			window.location = "bulletinAdd.html" ;
		});
