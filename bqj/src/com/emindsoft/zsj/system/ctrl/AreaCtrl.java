...
public class AreaCtrl extends AdminBaseController<Area> {
	...
	public void edit() {
		...
		try {
			...
			if( rearea!=null && rearea.getInt("demonstration")==null){
				rearea.set("demonstration",0);
			}
			if(rearea!=null && (rearea.getInt("demonstration")!=area.getInt("demonstration") || !rearea.getStr("Region").equals(area.getStr("Region")))){
				User u=getCurrentUser();
				if (!u.getStr("UserName").equals("admin")) {
					this.rendJson(false, null, "不是管理员，没有权限修改区域名和’示范‘状态");
					return;
				}
			}
