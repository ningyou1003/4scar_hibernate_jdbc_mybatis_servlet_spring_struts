(function(factory) {
    ...
    $.validator.config({
        ...
        rules: {
        	positive:[/^[0-9]+$/,"请输入正整数"],
        	negative:[/^-[0-9]+$/,"请输入负整数"],
        	positiveF:[/^[0-9]+(.[0-9]{1,})$/,"请输入正数"],
        	negativeF:[/^-[0-9]+(.[0-9]{1,})$/,"请输入负数"],
            digits: [/^\d+$/, "请输入数字"],
            number: [/^-{0,1}\d+(.[0-9]{1,})?$/, "请输入数字"],//含正负数小数
		    money: [/^(-)?(([1-9]{1}\d*)|([0]{1}))(\.(\d){1,2})?$/, "请输入金额"]
            ,letters: [/^[a-zA-Z]+$/i, "请输入字母"]
...
//            ,IDcard: [/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[A-Z])$/, "请输入正确的身份证号码"]
    ,IDcard: [/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[A-Z])$/, "请输入正确的身份证号码"]
            ,tel: [/^(?:(?:0\d{2,3}[\- ]?[1-9]\d{6,7})|(?:[48]00[\- ]?[1-9]\d{6}))$/, "请输入有效的固话号码"]
            ,mobile: [/^1[3-9]\d{9}$/, "请输入有效的手机号码"]
    		,contact:[/^1\d{10}$|^(0\d{2,3}-?|\(0\d{2,3}\))?[1-9]\d{4,7}(-\d{1,8})?$/,"请输入有效的手机或固话"]
            ,zipcode: [/^\d{6}$/, "请检查邮政编码格式"]
            ,chinese: [/^[\u0391-\uFFE5]+$/, "请输入中文字符"]
            ,username: [/^\w{3,12}$/, "请输入3-12位数字、字母、下划线"]
            ,password: [/^[\S]{6,16}$/, "请输入6-16位字符，不能包含空格"]
    		,fax: [/^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})+$/, "请输入有效的传真号"]
    		,e_name:[/^[A-Za-z][A-Za-z\s]*[A-Za-z]$/,"请输入正确格式的名称"]
