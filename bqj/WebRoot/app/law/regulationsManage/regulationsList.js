...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var regulationsPage = $.cookie('regulationsPageNumber');
		regulationsPage==null?1:regulationsPage;
		
		Public.ajaxPost(Public.rootPath() + '/regulations/regulationsList', {
	...
	initPage : function(pageData) {
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				// 存储页码到cookie 
            	$.cookie('regulationsPageNumber', index, {path: '/'});
...
/*		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "regulationsAdd.html?keyid=" + vm.selectId[0];
				} else {
		...
		$("#add").click(function(t) {// 添加
			window.location = "regulationsAdd.html" ;
		});
