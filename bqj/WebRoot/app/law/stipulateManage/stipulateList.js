...
THISPAGE = {
	...
	loadGrid : function() {
		...
		//从cookie获取页码
		var stipulatePage = $.cookie('stipulatePageNumber');
		stipulatePage==null?1:stipulatePage;
		
		Public.ajaxPost(Public.rootPath() + '/stipulate/stipulateList', {
			'page' : stipulatePage,
		...
		$("#PageInfo").pagePlugin({
			...
			requst : function(index) {
				Public.ajaxPost(Public.rootPath() + '/stipulate/stipulateList', {
					...
					// 存储页码到cookie 
	            	$.cookie('stipulatePageNumber', index, {path: '/'}); 
...
/*		$("#edit").click(function(t) {// 查看
			if (vm.selectId.length == 0) {
				...
				if (vm.selectId.length == 1) {
					window.location = "stipulateAdd.html?keyid=" + vm.selectId[0];
				} else {
		...
		$("#add").click(function(t) {// 添加
			window.location = "stipulateAdd.html" ;
		});
